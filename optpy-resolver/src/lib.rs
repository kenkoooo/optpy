...
pub fn resolve(statements: &[Statement]) -> Result<Vec<Statement>> {
    ...
    let statements = resolve_names(statements, &variables, &functions, &ctx)?;
    Ok(statements)
}
    ...
    statements
        ...
        .map(|s| match s {
            ...
            Statement::If { test, body, orelse } => {
                ...
                let body = resolve_names(body, variables, functions, ctx)?;
                let orelse = resolve_names(orelse, variables, functions, ctx)?;
